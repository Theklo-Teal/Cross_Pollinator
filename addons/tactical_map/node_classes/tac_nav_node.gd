...
signal entities_changed(added_entities:Array[TacEntity], removed_entities:Array[TacEntity])
...
@export var tile_height : float = 2.0  ## How high walls can get, if you neglect any margin for floor or ceiling thickness.

var entities : Array[TacEntity]  ## Reference to placed entities.
...
func _enter_tree() -> void:
	...
	if scene is TacInterface:
		scene._tacnav_entered(self)
func _exit_tree() -> void:
	var scene = get_tree().current_scene
	if scene is TacInterface:
		scene._tacnav_exited(self)
...
func _process(_delta: float) -> void:
	...
	if not entity_queue.is_empty():
		# Acknowledge Entities
		for entity in entity_queue:
			var cell = entity.get_nav_coord3()
			if entity_queue[entity] == true:
				entity.queue_free()
				entities.erase(entity)
				unblock_navigation(Vector2i(cell.x, cell.z), cell.y)
			else:
				if not entity in entities:
					entities.append(entity)
				block_navigation(Vector2i(cell.x, cell.z), cell.y)
		entities_changed.emit(
			entity_queue.keys().filter(func(entity):return not entity_queue[entity]),
			entity_queue.keys().filter(func(entity):return entity_queue[entity]),
			)
		entity_queue.clear()

## Add entity as child or flag it for acknowledgement if it's already a child.[br]
## If the entity was removed from the scene, it will forget about it.
func add_entities(...entities):
	for entity : TacEntity in entities:
		var parent = entity.get_parent()
		entity_queue[entity] = false
		if parent == null:
			add_child(entity)
func rem_entities(...entities):
	for entity : TacEntity in entities:
...
var entity_queue : Dictionary[TacEntity, bool]  # [entity] -> flag_remove ; TacEntities found as children or generated by spawners.
func _ready() -> void:
	...
	if not OS.has_feature("editor_hint"):
		...
		add_entities.callv(found_entities)
